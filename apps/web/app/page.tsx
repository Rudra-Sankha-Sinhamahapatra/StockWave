...
export default function Page() {
  const { registerUser, sendMessage, messages, error } = useSocket();
  ...
  const handleRegister = () => {
    if (name.trim() === "") {
      setInputError("Please enter at least one letter.");
      return;
    }
    setInputError("");
    registerUser(name);
        ...
        <>
          <div className="flex-grow ml-20">
            ...
            <h2 className="text-white font-bold">
              Server: {serverMsg} <span className="text-blue-400">{name}</span>
            </h2>
            <ul className="text-white font-semibold mb-2">
